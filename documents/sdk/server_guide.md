...
### 1.1.2. 流程说明
> A. 游戏客户端需要调用登录sdk，用户通过登录sdk注册登录>
> b. 用户登录成功后，登录sdk服务器会返回包含entity和sign的`验证结果`>
> c. 游戏客户端需要通过登录sdk取得`验证结果`并提交到游戏服务器>
> d. 游戏服务器使用游戏客户端提交的entity和sign以及预先配置的公钥（公钥由我们提供）进行rsa签名验证>
> e. 验证通过则登录流程结束，游戏服务器就可以通过entity中的openid进行创建角色或者获取角色信息等游戏业务逻辑>
